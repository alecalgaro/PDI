...
def draw_predictions(image, xlims, ylims, ymin, ymax, percs):
    ...
    fig.suptitle("Filled bottles", fontsize=16)
    ax1.imshow(image)
    ax1.axis("off")
    ax1.set_title(f"{args.d}botellas_{args.n:02}.tif")
    plt.show()


desc="Detects non-filled bottles."
parser = argparse.ArgumentParser(description=desc)
parser.add_argument("-n", "--number", dest="n", default=1, type=int,
                    help="Image ID (1 by default)")
parser.add_argument("-d", "--directory", dest="d", default="botellas/",
                    help="Images directory (botellas/ by default)")
args = parser.parse_args()
imagen = cv2.imread(f"{args.d}botellas_{args.n:02}.tif", cv2.IMREAD_GRAYSCALE)
xlims = detect_bottles(imagen)
ylims, min_y = detect_filled_level(imagen, xlims)
percs, max_y = compute_percentages(ylims, min_y)
draw_predictions(imagen, xlims, ylims, min_y, max_y, percs)
