...
image = cv2.imread(PATH + "rmn.jpg", cv2.IMREAD_GRAYSCALE)
...
# Como el log tiene valores chicos, por ejemplo, log(255) = 2.40, entonces el valor de "c" deberia 
# ser grande para que la imagen se vea bien. Algo que se hace es normalizar el rango de 0-255
# para pasarlo a 0-1, aplicar el log y luego volver a escalarlo al rango 0-255.
def apply_log_transformation(img, LUT, c):
    # Normalizar para tener un rango 0-1
    LUT_normalized = LUT / 255

    # Aplicar la transformación logarítmica
    new_LUT = c * np.log(1 + LUT_normalized)
